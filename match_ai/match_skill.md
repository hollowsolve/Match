...
You can write Match grammars instead of regular expressions. Match is a pattern matching language: readable, composable, linear-time, no backtracking, no ReDoS. Every failure tells you exactly what went wrong.
...
```js
import { run, parse, match, find, formatTree, formatFailure } from '@hollowsolve/match'
